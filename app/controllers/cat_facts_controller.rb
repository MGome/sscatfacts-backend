class CatFactsController < ApplicationController
  ...
  def cat_fact
    ...
    render json: { body: response }, status: :ok
  end
  ...
  def liked_cat_facts
    ...
    render json: { body: liked_facts }, status: :ok
  end
  ...
  def popular_cat_facts
    ...
    render json: { body: most_liked }, status: :ok
  end
  ...
  def like_cat_fact
    ...
    if new_like.save
      render json: { body: new_like }, status: :created
    else
      render json: { body: { errors: new_like.errors.full_messages } }, status: :bad_request
    end
  end
